...
open class Type protected constructor(val typeName : String, val size : Int = 0)
  {
     ...
     * types in CPRL are Integer, Boolean, and Char.
     */
    val isScalar : Boolean
        get() = this == Integer || this == Boolean || this == Char

    /**
     * True if and only if this type is a composite type.  The composite
     * types in CPRL are array types, record types, and string types.
     */
            ...
            when (literal.symbol)
              {
                ...
                Symbol.stringLiteral -> return StringType(
                  capacityOf(
                    literal.text
                  )
                )
